...
signed main(){
    ...
    ll n,m; cin>>n>>m;
    ll tot=n*n;
    unordered_map<ll,bool> mp1,mp2;
    while(m--){
        int row,col; cin>>row>>col;
        if(!mp1.count(row) and !mp2.count(col) and tot>0)
            tot-=2*n-1-mp1.size()-mp2.size();
        else if(!mp1.count(row))
            tot-=n-mp2.size();
        else if(!mp2.count(col))
            tot-=n-mp1.size();
        cout<<tot<<" ";
        mp1[row]=mp2[col]=true;
    }
return 0;
}
